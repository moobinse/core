	...
	public static void generateCacheNavigator(String p) {
		...
		try (BufferedWriter writer = Files.newBufferedWriter(path, charset)) {
			...
			for (MetaDataObject<?> meta : Core.get().getMetaDataManager().getMetaData()) {
				...
				writer.write("    }\n");
				writer.write("    public static void get" + meta.getName() + "List(Collection<String> keys, CacheCallback<JsArray<Js" + meta.getName() + ">> callback) {\n");
			    ...
			    for (MetaDataField<?, ?> f : meta.getFields()) {
					...
					if (f.isArray()) {
						if (f.getJavaType() == int.class) {
							writer.write("    public JsArrayInteger get" + name + "() {\n");
							writer.write("        return getArrayInteger(\"" + n + "\");\n");
							writer.write("    }\n");
							writer.write("    public void " + " set" + name + "(JsArrayInteger " + n + ") {\n");
						}
						else if (f.getJavaType() == boolean.class) {
							writer.write("    public JsArrayBoolean get" + name + "() {\n");
							writer.write("        return getArrayBoolean(\"" + n + "\");\n");
							writer.write("    }\n");
							writer.write("    public void " + " set" + name + "(JsArrayBoolean " + n + ") {\n");
						}
						else if (f.getJavaType() == String.class) {
							writer.write("    public JsArrayString get" + name + "() {\n");
							writer.write("        return getArrayString(\"" + n + "\");\n");
							writer.write("    }\n");
							writer.write("    public void " + " set" + name + "(JsArrayString " + n + ") {\n");
						}
						else {
							writer.write("    public JsArray<Js" + f.getJavaType().getSimpleName() + "> get" + name + "() {\n");
							writer.write("        return getArray(\"" + n + "\");\n");
							writer.write("    }\n");
							writer.write("    public void " + " set" + name + "(JsArray<Js" + f.getJavaType().getSimpleName() + "\" v) {\n");
						}
						writer.write("        set(\"" + n + "\", " + n + ");\n");
						writer.write("    }\n");
					}
					else {
						writer.write("    public " + type + " get" + name + "() {\n");
						if (f.getType() == Type.OBJECT) {
						...
						writer.write("    }\n\n");
						writer.write("    public void " + " set" + name + "(" + type + " " + n + ") {\n");
						writer.write("        set(\"" + n + "\", " + n + ");\n");
						writer.write("    }\n\n");
						Class<?> ref = f.getReferenceType();
						if (ref != null) {
							writer.write("    public void get" + name + "(CacheCallback<Js" + ref.getSimpleName() + "> callback) {\n");
